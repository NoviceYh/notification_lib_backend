...
public class SendNotificationUseCase implements NotificationService {
    ...
    public DeliveryResult send(NotificationRequest request) {
        if (request == null) throw new ValidationException("La notificación no puede ser null");
        if (request.channel() == null) throw new ValidationException("El channel es obligatorio");

        ChannelSender sender = senderByChannel.get(request.channel());
        if (sender == null) {
            throw new ValidationException("No hay sender configurado para channel " + request.channel());
        }
